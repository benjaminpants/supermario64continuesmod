...
void update_mario_health(struct MarioState *m) {
    ...
    if (m->health >= 0x100) {
        ...
        if (m->health < 0x100) { //When the player is going to die/about to die
			if (gMarioState->numLives > 0) { //If you still have "continues" left
			gMarioState->numLives -= 1; //Remove a continue
			m->health = 0x880; //Set health to max
			}
			if (gMarioState->numLives < 0) { //If they have no continues left
			gMarioState->numLives -= 1; //Remove a continue
			m->health = 0xFF; //Let them actually die properly so they can goto the game over screen.
			}
        }
